  ...
  function HFUpload(props, context) {
    ...
    _this.beforeUpload = function (file) {
      ...
      if (_this.state.files.length >= limit) {
        ...
        _message2.default.error('上传文件数量超过限制!' + limit);
        return false;
      }

      // 默认10M
      var _this$props$fileSize = _this.props.fileSize,
          fileSize = _this$props$fileSize === undefined ? 1024 * 1024 * 10 : _this$props$fileSize;

      if (file.size > fileSize) {
        _message2.default.error('上传文件大小超过限制!' + fileSize);
        return false;
      }
    ...
    _this.oss = new _oss_client2.default(baseDir, renderType, mode, keyId, custom);
